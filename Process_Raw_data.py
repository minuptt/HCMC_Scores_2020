...
for data in datas:
    ...
    # replace web code to charactoer in name
    for i in range (len(name)):
        if name[i:i+2] == "&#":
            name = name[:i] + chr(int(name[i+2:i+5])) + name[i+6:]

    # repalce web code to charactor in scores
    ...
    data = [std_id_str, name.title(), str(dd), str(mm), str(yy)]
    # add score to data
